...
(function (root, factory) {
    ...
    LineInstrumentor.prototype.propertyLocationOf = function (memberExpression) {
        ...
        if (typeof prop.loc !== 'undefined') {
            ...
            if (computed) {
                startObjectLiteralNode = propLocation.properties[0].value;
    ...
    LineInstrumentor.prototype.findLeftBracketTokenOf = function (expression) {
        var i, token, prevToken,
            startOfRightColumn = expression.property.loc.start.column,
            startOfRightLine = expression.property.loc.start.line,
            endOfLeftLine = expression.loc.start.line;
        for(i = 0; i < this.tokens.length; i += 1) {
            token = this.tokens[i];
            if (token.loc.start.line < endOfLeftLine) {
                continue;
            }
            if (startOfRightLine < token.loc.end.line) {
                break;
            }
            if (token.loc.start.column === startOfRightColumn) {
    ...
    LineInstrumentor.prototype.findOperatorTokenOf = function (expression) {
        var i, token,
            endOfLeftColumn = expression.left.loc.end.column,
            startOfRightColumn = expression.right.loc.start.column,
            endOfLeftLine = expression.left.loc.end.line,
            startOfRightLine = expression.right.loc.start.line;
        for(i = 0; i < this.tokens.length; i += 1) {
            token = this.tokens[i];
            if (token.loc.start.line < endOfLeftLine) {
                continue;
            ...
            if (endOfLeftColumn < token.loc.start.column && token.loc.end.column < startOfRightColumn && token.value === expression.operator) {
                return token;
            }
        }
        return undefined;
