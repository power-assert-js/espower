    ...
    SourceInstrumentor.prototype.instrument = function (ast) {
        ...
        estraverse.replace(result, {
            enter: function (currentNode, parentNode) {
                ...
                console.log('enter ' + currentNode.type + ' path: ' + path);

                if(parentNode.type === syntax.AssignmentExpression && currentPath === 'left') {
                    // Do not instrument left due to 'Invalid left-hand side in assignment'
                    //console.log('skip ' + currentNode.type + ' path: ' + path + ' ');
                    skipping = true;
                    this.skip();
                    return;
                ...
                if (capturing !== '' && !isSupportedNodeType(currentNode)) {
                    skipping = true;
                    this.skip();
                    return;
                }
            ...
            leave: function (currentNode, parentNode) {
                ...
                // if (isSupportedNodeType (parentNode)) {
                //     return result;
                // }
                if (parentNode.callee === currentNode) {
                    return result;
                ...
                case syntax.Identifier:
                    ...
                    // if (currentPath === 'property' && path[path.length - 2] === 'callee' && parentNode.computed) {
                    //     // property of CallExpression
                    if (parentNode.type === syntax.MemberExpression && !parentNode.computed && currentPath === 'property') {
                        // non-computed MemberExpression
                    } else if (parentNode.type === syntax.UnaryExpression && (parentNode.operator === 'typeof' || parentNode.operator === 'delete') && currentPath === 'argument') {
                        // 'typeof Identifier' or 'delete Identifier' is not instrumented
                    } else if (parentNode.type === syntax.AssignmentExpression && currentPath === 'left') {
                        //console.log('leave ' + currentNode.type + ' path: ' + path + ' ' + currentNode.name);
                        // Do not instrument left due to 'Invalid left-hand side in assignment'
                    } else {
                        //return captureNode(currentNode, path);
                        //console.log('leaving ' + capturing);
                        result = instrumentor.captureNode('ident', currentNode, instrumentor.locationOf(currentNode));
                    }
                    break;
                case syntax.MemberExpression:
                    propLocation = instrumentor.propertyLocationOf(currentNode);
                    result = instrumentor.captureNode('ident', currentNode, propLocation);
                    break;
                case syntax.CallExpression:
                    if (currentNode.callee.type === syntax.MemberExpression) {
                        result = instrumentor.captureNode('funcall', currentNode, instrumentor.propertyLocationOf(currentNode.callee));
                    } else {
                        result = instrumentor.captureNode('funcall', currentNode, instrumentor.locationOf(currentNode));
                    }
                    break;
                case syntax.UnaryExpression:
                    result = instrumentor.captureNode('unary', currentNode, instrumentor.locationOf(currentNode));
                    break;
                case syntax.BinaryExpression:
                    infixOperatorLocation = instrumentor.infixOperatorLocationOf(currentNode);
                    result = instrumentor.captureNode('binary', currentNode, infixOperatorLocation);
                    break;
                case syntax.LogicalExpression:
                    infixOperatorLocation = instrumentor.infixOperatorLocationOf(currentNode);
                    result = instrumentor.captureNode('logical', currentNode, infixOperatorLocation);
                    break;
                case syntax.AssignmentExpression:
                    infixOperatorLocation = instrumentor.infixOperatorLocationOf(currentNode);
                    result = instrumentor.captureNode('assignment', currentNode, infixOperatorLocation);
                    break;
                case syntax.UpdateExpression:
                    result = instrumentor.captureNode('update', currentNode, instrumentor.locationOf(currentNode));
                    break;
                case syntax.NewExpression:
                    result = instrumentor.captureNode('new', currentNode, instrumentor.locationOf(currentNode));
                    break;
                ...
                if (result && path && capturing === path.join('/')) {
                    //console.log('leaving ' + capturing);
                    capturing = '';
                    return instrumentor.captureAssertion(result, instrumentor.locationOf(currentNode));
                }
