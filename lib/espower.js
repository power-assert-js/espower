/**
 * espower.js - Instrument power assert feature into code
 *
...
Instrumentor.prototype.captureRecursively = function (node) {
    ...
    case 'UnaryExpression':
        ...
        return that.captureUnary(node, that.locationOf(node));
    case 'BinaryExpression':
        infixOperatorLocation = that.infixOperatorLocationOf(node); // BK: need to detect operator location before left/right instrumentation
        node.left = that.captureRecursively(node.left);
        node.right = that.captureRecursively(node.right);
        return that.captureBinary(node, infixOperatorLocation);
    case 'AssignmentExpression':
        infixOperatorLocation = that.infixOperatorLocationOf(node); // BK: need to detect operator location before left/right instrumentation
        // node.left = that.captureRecursively(node.left);  // Do not instrument left due to 'Invalid left-hand side in assignment'
        node.right = that.captureRecursively(node.right);
        return that.captureAssignment(node, infixOperatorLocation);
    case 'LogicalExpression':
        infixOperatorLocation = that.infixOperatorLocationOf(node); // BK: need to detect operator location before left/right instrumentation
        node.left = that.captureRecursively(node.left);
        node.right = that.captureRecursively(node.right);
        return that.captureLogical(node, infixOperatorLocation);
    default:
        return node;
    }
};
...
Instrumentor.prototype.captureUnary = function (target, location) {
    return this.captureNode('unary', target, location);
};

Instrumentor.prototype.captureAssignment = function (target, location) {
    return this.captureNode('assignment', target, location);
};

Instrumentor.prototype.captureLogical = function (target, location) {
    return this.captureNode('logical', target, location);
};
