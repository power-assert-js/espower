...
AssertionVisitor.prototype.createNewRecorder = function (controller) {
    var _this = this;
    var found = findEnclosingBlockStatement(controller.parents(), controller.path());
    ...
    this.options.storage.transformation.register(argumentEspath, function (matchNode) {
        var init = _this.createNewExpression(n, '_powerAssertRecorder');
        matchNode.body.unshift(_this.createVariableDeclaration(n, '_rec', init));
    });
};

function findEnclosingBlockStatement (parents, initialPath) {
    ...
    for (var i = parents.length - 1; i >= 0; i--) {
        ...
        if (parent.type === syntax.BlockStatement) {
