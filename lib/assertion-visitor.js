...
AssertionVisitor.prototype.createNewRecorder = function (controller) {
    ...
    var found = findEnclosingScopeEspath(controller.parents(), controller.path(), currentScope.block);
    var argumentEspath = found.path.join('/');
    // var helperNameNode = this.getRecordHelperNameNode(controller);

    var currentNode = controller.current();
    var createNode = newNodeWithLocationCopyOf(currentNode);
    var init = this.createNewExpression(createNode, '_powerAssertRecorder');
    var decl = this.createVariableDeclaration(createNode, '_rec', init);
    this.options.storage.transformation.register(argumentEspath, function (matchNode) {
        if (/Function/.test(matchNode.type)) {
            var blockStatement = matchNode.body;
            blockStatement.body.unshift(decl);
        } else {
            matchNode.body.unshift(decl);
        }
    });
};

function findEnclosingScopeEspath (parents, initialPath, scopeNode) {
    // iterate child to root
    var child, parent, path = initialPath;
    for (var i = parents.length - 1; i >= 0; i--) {
        parent = parents[i];
        if (child) {
            function popUntilParent (key) {
...
AssertionVisitor.prototype.createVariableDeclaration = function (createNode, varName, init) {
    return createNode({
